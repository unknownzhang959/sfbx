...
public class TaskExecutorConfig {
     ...
     * @return
     */
    @Bean("browsingHistoryExecutor")
    public Executor browsingHistoryExecutor() {
        log.info("start browsingHistoryExecutor");
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        //配置核心线程数
        executor.setCorePoolSize(taskExecutorProperties.getCorePoolSize());
        //配置最大线程数
        ...
        executor.setQueueCapacity(taskExecutorProperties.getQueueCapacity());
        //配置线程池中的线程的名称前缀
        executor.setThreadNamePrefix("browsingHistoryExecutor");
        // rejection-policy：当pool已经达到max size的时候，如何处理新任务
        // CALLER_RUNS：不在新线程中执行任务，而是有调用者所在的线程来执行
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy());
        //执行初始化
     ...
     */
    @Bean("insureExecutor")
    public Executor insureExecutor() {
        log.info("start insureExecutor");
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        //配置核心线程数
        executor.setCorePoolSize(taskExecutorProperties.getCorePoolSize());
        //配置最大线程数
        ...
        executor.setQueueCapacity(taskExecutorProperties.getQueueCapacity());
        //配置线程池中的线程的名称前缀
        executor.setThreadNamePrefix("insureExecutor");
        // rejection-policy：当pool已经达到max size的时候，如何处理新任务
        // CALLER_RUNS：不在新线程中执行任务，而是有调用者所在的线程来执行
        executor.setRejectedExecutionHandler(new ThreadPoolExecutor.CallerRunsPolicy());
        //执行初始化
